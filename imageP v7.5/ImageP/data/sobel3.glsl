...
void main(void) {
  ...
  horizEdge -= texture2D( texture, vec2( vertTexCoord.x - x, vertTexCoord.y     ) ) * mult1;
  horizEdge -= texture2D( texture, vec2( vertTexCoord.x - x, vertTexCoord.y + y ) ) * 1.0;
  horizEdge += texture2D( texture, vec2( vertTexCoord.x + x, vertTexCoord.y - y ) ) * 1.0;
  horizEdge += texture2D( texture, vec2( vertTexCoord.x + x, vertTexCoord.y     ) ) * mult1;
  horizEdge += texture2D( texture, vec2( vertTexCoord.x + x, vertTexCoord.y + y ) ) * 1.0;
  ...
  vertEdge -= texture2D( texture, vec2( vertTexCoord.x    , vertTexCoord.y - y ) ) * mult1;
  vertEdge -= texture2D( texture, vec2( vertTexCoord.x + x, vertTexCoord.y - y ) ) * 1.0;
  vertEdge += texture2D( texture, vec2( vertTexCoord.x - x, vertTexCoord.y + y ) ) * 1.0;
  vertEdge += texture2D( texture, vec2( vertTexCoord.x    , vertTexCoord.y + y ) ) * mult1;
  vertEdge += texture2D( texture, vec2( vertTexCoord.x + x, vertTexCoord.y + y ) ) * 1.0;
  ...
  float sum = (edge.r+edge.g+edge.b)/(3.0);
	...
	// if(y1<0||x1<0) gl_FragColor = vec4(n-x2,n-y2,1.0-mag*mult,1.0);
	// else 
	 gl_FragColor = vec4(x2,y2,1.0-mag*mult,1.0);
	// gl_FragColor = vec4(n-x2,n-y2,1.0-mag*mult,1.0);
		
	//gl_FragColor = vec4(1.0,1.0,1.0-mag*mult*2.0,1.0);
