...
void main(void) {
  ...
  float myCol = mCol.b;
		...
		if ((0.0 <= myGrad && myGrad < (22.5))){
			p1 = texelFetch(texture, ivec2( vertTexCoord.x + x, vertTexCoord.y ), 0.0);
			p2 = texelFetch(texture, ivec2( vertTexCoord.x - x, vertTexCoord.y ), 0.0);
			q = p1.b;
			r = p2.b;
		}
		//angle 45
		else if ((22.5) <= myGrad && myGrad < (67.5)){
			p1 = texelFetch(texture, ivec2( vertTexCoord.x + x, vertTexCoord.y + y ), 0.0);
			p2 = texelFetch(texture, ivec2( vertTexCoord.x - x, vertTexCoord.y - y ), 0.0);
			q = p1.b;
			r = p2.b;
		}
		//angle 90
		else if ((67.5) <= myGrad && myGrad< (112.5)){
			p1 = texelFetch(texture, ivec2( vertTexCoord.x, vertTexCoord.y + y ), 0.0);
			p2 = texelFetch(texture, ivec2( vertTexCoord.x, vertTexCoord.y - y ), 0.0);
			q = p1.b;
			r = p2.b;
		}
		//angle 135
		...
		//angle 360
		else if ((337.5) <= myGrad && myGrad <= (360.0)){
			p1 = texelFetch(texture, ivec2( vertTexCoord.x + x, vertTexCoord.y ), 0.0);
			p2 = texelFetch(texture, ivec2( vertTexCoord.x - x, vertTexCoord.y ), 0.0);
			q = p1.b;
			r = p2.b;
