...
void main(void) {
  ...
  float sobelA_Strength = GetStrength(sorbeA);
  ...
  col += mix(sourceColor, texture(GREENSCREEN_CHANNEL, uv), 1.-fac);
    ...
    #if COLOR_ANGLE
    vec3 outlineCol = vec3(sin(angle), sin(angle + 2.0*M_PI/3.0), sin(angle + 2.0*M_PI/3.0 * 2.0));
  ...
  // Output to screen
  //col = vec4(angle,angle,angle,1.0);
  gl_FragColor = col;
}
